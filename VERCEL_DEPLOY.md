...
- **Note**: This is client-side protection. The password will be visible in the built JavaScript bundle (VITE\_ vars are exposed to frontend). For stronger security, consider Vercel Password Protection (Pro plan) or backend authentication.
...
### Password protection shows "not configured" error?

- Make sure `VITE_APP_PASSWORD` is set in Vercel dashboard
- **Important**: After setting/updating env vars, you MUST redeploy:
  - Go to Deployments tab
  - Click the 3 dots on latest deployment → "Redeploy"
  - Or push a new commit to trigger auto-deploy
- Check that env var is set for **Production** environment (not just Preview/Development)
- Verify the variable name is exactly `VITE_APP_PASSWORD` (case-sensitive)

### Need to update password?

- Change `VITE_APP_PASSWORD` in Vercel dashboard
- **Redeploy** (go to Deployments → Redeploy, or push a new commit)
